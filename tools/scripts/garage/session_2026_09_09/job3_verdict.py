...
camera the scene is composed for. Three pictures out:
  ...
  job3_off.png    the shipped look
  job3_on.png     the contract's
  job3_diff.png   where they differ, amplified -- red where the contract is
...
for name, cam in CAMS.items():
    off = shot('job3_%s_off' % name, cam, ['--water-contract=off'])
    on = shot('job3_%s_on' % name, cam, ['--water-contract=on'])
    if not (off and on):
    ...
    Image.fromarray(diff).save(os.path.join(SHOTS, 'job3_%s_diff.png' % name))
