# -*- coding: utf-8 -*-
"""RT-8 job 3: the sea's own averaging against the contract's.
...
for name, cam in CAMERAS.items():
    p = run('j3_mask_%s' % name, cam, extra=['--debug-view=water-mask'])
    if not p:
...
# --- A/B/B/A ------------------------------------------------------------
ARMS = {'own': ['--water-contract=off'], 'contract': ['--water-contract=on']}
order = ['own', 'contract', 'contract', 'own']
shots = {}
for name, cam in CAMERAS.items():
    for i, arm in enumerate(order):
        shots.setdefault((name, arm), []).append(
            run('j3_%s_%s_%d' % (name, arm, i), cam, extra=ARMS[arm]))
