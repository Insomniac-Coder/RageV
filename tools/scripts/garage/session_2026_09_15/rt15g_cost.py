# -*- coding: utf-8 -*-
"""What RT-15's four fixes cost (2026-09-16 night, laptop on mains).
...
# The four fixes alone. Three are flags; the direct light's object test is engine code, so the
# 'before' arm stages that line out the way any measurement arm stages anything else.
OLD_GUARD = '#if !defined(RV_SIGNAL_DIFFUSE)' + NL + chr(9) + chr(9) + "// **RT-15c: another object's history is never this one's.**"
NEW_GUARD = '#if !defined(RV_SIGNAL_PAIR) && !defined(RV_SIGNAL_DIFFUSE)' + NL + chr(9) + chr(9) + "// **RT-15c: another object's history is never this one's.**"
stage_run.VARIANTS['fixesoff'] = [(ACC, OLD_GUARD, NEW_GUARD)]
ARMS = {'before': ('fixesoff', ['--reflection-moving-blur=0', '--reflection-firefly=0', '--reflection-noise-blur=off']),
        'ship': ('ship', [])}
    ...
    try:
        ...
        for scene, slide in (('driving', '=porsche_992_gt3_r|0.5|12.0'), ('parked', None)):
            ...
            for i, arm in enumerate(['before', 'ship', 'ship', 'before']):
                ...
                io.open(os.path.join(OUT, 'rt15g_cost_%s_%d_%s.log' % (scene, i, arm)), 'w', encoding='utf-8').write(text)
