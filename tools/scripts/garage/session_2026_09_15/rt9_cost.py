...
# RT-9: the rays allocated by what the history says, against RT-15b's motion rule.
# RT-9: the rays allocated per tile by what the history says, against RT-15b's motion rule.
ARMS = {'before': ('ship', []),
        'ship': ('ship', ['--reflection-confidence-rays=on'])}
    ...
    try:
        ...
        for scene, slide in (('driving', '=porsche_992_gt3_r|0.5|12.0'), ('parked', None)):
            ...
            for i, arm in enumerate(['before', 'ship', 'ship', 'before']):
                ...
                io.open(os.path.join(OUT, 'rt9_cost_%s_%d_%s.log' % (scene, i, arm)), 'w', encoding='utf-8').write(text)
