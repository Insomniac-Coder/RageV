...
# The demo's two imported models, one per format, and each is the object in
# the scene that format is the right answer for. The fox is a rigged character
# from an online library, which is glTF's world; the anvil is a modelled prop
# out of a DCC, which is FBX's.
MESH_FOX = 11679010045657754579
MESH_ANVIL = 16802294582248143889

# The hillside the forge stands on. Generated by make_terrain.py with its
# layer weights already painted, so the demo only chooses what the layers are
# made of.
TERRAIN_HILLS = 15731441207428090258
...
def build(profile_handle, mat):
    ...
    # **The demo's FBX asset**, and it is here rather than anywhere else
    # because this is the object a box was already failing to describe. An
    # anvil has a horn, a waist and a step; a cube has none of them, and all
    # three are visible from where the camera stands.
    #
    # Modelled by make_demo_models.py, exported as FBX, and imported through
    # the same door the fox comes through (8.9). The rusted steel maps rather
    # than a flat grey: an anvil is the one object in a forge that has
    # obviously been *used*, and smooth untextured metal says the opposite.
    s.entity("Anvil", position=(-2.6, 0.6, -0.35), scale=(1, 1, 1))
    s.mesh(MESH_ANVIL, MAT_STEEL)
    s.script("Anvil")
    s.block("AudioSourceComponent", [
...
def main():
    ...
    mat = {
        ...
        # The hillside, over the same soil maps as the courtyard floor. Same
        # ground, seen from further away -- which is the argument for reusing
        # the map set rather than authoring a second one, and for the tiling
        # being the only thing that differs: a hill is two hundred metres of
        # the surface the yard has ten of.
        #
        # The terrain's own TextureScale multiplies these, so they are small
        # numbers here on purpose.
        "hill": write_material(materials / "courtyard_hill.rmat", MAPS["soil"],
                               tiling=(1.0, 1.0), height_scale=0.02),

        # And the drier grass the painted weights put on the tops. Wood maps
        # for the roughness and normal detail, tinted -- there is no grass map
        # in this project and inventing one is a texturing job, where a tinted
        # broken-up surface at two hundred metres is indistinguishable from it.
        "hill_grass": write_material(materials / "courtyard_hillgrass.rmat",
                                     MAPS["soil"], tiling=(1.0, 1.0),
                                     height_scale=0.01, roughness=0.95,
                                     base_color=(0.42, 0.46, 0.3, 1)),
    }
    ...
    profile = postprofile.write_named(assets / "post" / "courtyard.rvpostprofile", {
        ...
        # **Global illumination, and it belongs in the generator rather than
        # in the file.** It was switched on by hand in the editor and lost the
        # moment this script next ran, which is what a generated file does to
        # a hand edit -- the generator is the source and anything not in it is
        # temporary by construction.
        #
        # The screen-space form: it is the one that runs on both backends and
        # on hardware without ray queries, and it is not exclusive with
        # anything else in this profile. If Render Settings turns the traced
        # or voxel form on, this row greys out and says so (7ao).
