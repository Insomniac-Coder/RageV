...
namespace RageV
{
	...
	void Renderer2D::Init()
	{
		...
		renderer2DData->QuadVerticiesBuffer = new VertexData[renderer2DData->MaxQuads];
	...
	void Renderer2D::DrawQuad(Transform2D& transform, glm::vec4& color)
	{
		...
		renderer2DData->QuadVerticiesPtr->Position = glm::vec3(transform.position.x - (transform.scale.x / 2), transform.position.y - (transform.scale.y / 2), transform.position.z);
