...
### START HERE: two open bugs from 9.0/9.1, reported 2026-08-14

Both were found by the owner using the editor, after 9.1 landed. Neither is
diagnosed. Do these before any new phase-9 item.

---

**BUG 1 — a post profile picked for a camera, and a LUT applied to that
profile, are both gone after an editor restart.**

Repro, entirely in **Edit mode** (no Play involved -- an earlier guess that
this was Play/Stop snapshot loss is *wrong*, the owner ruled it out):

1. Select the scene camera. Pick a post profile on its Post profile row.
2. Set a Colour LUT on that profile (drawn inline under the row).
3. Restart the editor.
4. Both are back to what they were. They have to be applied again.

What is already known, so do not re-derive it:

- **The file format is not the problem.** `CheckPostProfileOnCamera` asserts
  the camera's handle round-trips, and `check_color_grading.py` renders three
  scenes differing only in which profile the camera names and gets three
  different grades. Both pass.
- **The inspector does record the edit.** `ReadFieldValue` handles
  `FieldType::Asset`, so before/after differ and `CommitPendingEdit` pushes a
  `FieldEditCommand`.
- **Writes are reaching disk, possibly the wrong file.**
  `SampleProject/assets/post/grade_none.rvpostprofile` was generated by
  `check_color_grading.py` with only `BloomEnabled: false`, and on disk now
  carries a full field list *plus* `ColorLut: 5920185463324807925` -- which is
  `swap.cube`, a LUT that profile was never given. The full field list with no
  trailing newline is `WriteFields`' output, so the **engine** wrote it. No
  handle collisions: all five `.meta` handles are distinct. **Start here.**

The two things not yet checked:

- Does the unsaved-changes mark in the title bar light up for this edit? The
  camera's profile handle is a *scene* edit and needs Ctrl+S; the profile's own
  fields save immediately to the `.rvpostprofile`. If the mark does not appear,
  that asymmetry is invisible and is the bug.
- Is `DrawPostProfile` ever saving to a path other than the handle it was
  given? That is the only way grade_none acquires swap.cube's handle.

**The design wart underneath is mine and worth fixing either way:** one gesture
("give this camera this look") persists on two different schedules, because the
profile is an asset and the assignment is scene data. However it is explained,
it reads as "it did not save".

---

**BUG 2 — a faint ghost image over the Game view at startup.**

- Present **as soon as the editor opens**.
- **Game view only.** The Viewport panel is clean.
- The **scroll wheel zooms it** at first -- which is the editor camera's
  control, and nothing in the Game view should answer to it.
- Switch to the Viewport tab and back to Game: **the ghost is still there but
  no longer responds to scroll.**

That last behaviour is the diagnostic one. It reads as a *stale image left in a
target* rather than a live second render: something the editor viewport drew is
reaching the Game panel's target, and once the Viewport panel has drawn a frame
of its own the stale copy stops updating.

Where to look, in order: the two frame chains in EditorLayer (`m_Graph` /
`m_GameGraph`, `m_SceneTarget` / `m_GameTarget`), whether the Game target is
cleared on the frames before the game camera first renders, and the render
graph's *pooled* targets -- they are reused by shape within a frame, and the
two chains have the same shapes.

One cheap first step: does it survive `--aa=none`? If it goes away, the shared
suspect is the TAA history (the editor has two, one per viewport); if it
persists, it is the targets themselves. Neither commit touched that path, so
this may predate 9.0.
