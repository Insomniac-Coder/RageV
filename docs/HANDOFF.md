...
**4. A defect in the verification tooling, found while starting the
local-light follow-up.** **Vulkan does not pick up edits to the deployed
`voxel_inject.rvshader`.** Scaling every injected voxel's radiance by 0.25 --
a change that cannot fail to alter the picture -- moves OpenGL from +0.43 to
+0.08 and leaves Vulkan bit-identical at +0.50; forcing the shadow term to 0
takes OpenGL to +0.00 and again leaves Vulkan alone. So `falsify.py`'s lever
does not reach that shader on that backend, and **CHK.2's "every claim has
been falsified" guarantee does not hold there.** How far it extends is
unknown. Start at `ShaderCompiler` and the cache hash 8.2b added. **This is
why the local-light shadow follow-up is not built** -- it would be
unverifiable on Vulkan. Details and the measurement table are in 7bc's fifth
finding.
...
**Next, the owner's call:** the **Vulkan shader-cache defect** above, which
now gates any further falsify work on compute shaders; or **9.15** as now specified (the march); or the
