...
out-of-grid half is a list that does not depend on the picture: the
world-space lamp grid, now part of S4's definition (decided 2026-09-04),
built once from the lamps' positions and serving every hit and every
sampler. A guard band of extra rows on the cluster grid was considered
and dropped: it would still miss the mirror hits behind the camera, about
a third of Pier's. Not worth building for the 2 ms alone; in S4 it is the
mechanism, not an optimisation. Headland's remaining
