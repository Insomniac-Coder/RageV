...
| **RT-24** | **open, HIGHEST PRIORITY (owner, 2026-09-23): before anything else.** One fast camera spin around the car smears the whole scene. Prime suspects (owner): the car and the chrome poles. Also (owner): on fast movements the ceiling tube lights' reflections on the car take a fraction of a second to update. First arm, agreed and not yet run: the owner spins by hand with the scene's own camera and `--reflection-history=off` -- gone means the reflection accumulator, still there means the frame filter on those surfaces. HANDOFF's top has it | not priced | high | a fast camera spin leaves no smear |
