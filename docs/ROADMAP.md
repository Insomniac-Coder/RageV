...
| 7.14 | SMAA's diagonal pass | M | A second search along the 45° axes with its own pattern classification. **Justified by measurement rather than by the paper**: at 43° SMAA is 1.6× against no filter where a shallow edge is 6.1×, because a near-diagonal run is one pixel long and the orthogonal reconstruction has almost nothing to work with. The first measurement said it was unnecessary and had been taken at exactly 45°, which is degenerate — ENGINE-NOTES §7n. Designed in §7p: two staircase orientations needing different searches, and one more exact coverage function rather than the reference's second lookup table |
