...
| 8.13 | Hybrid GI — rays for the first bounce, the voxel grid for the rest | M | **Designed 2026-08-19, not built (ENGINE-NOTES 7be).** The one design in which 8.1 and 8.12 cooperate instead of excluding each other. Today the traced form shoots four cosine rays and shades the first hit from `ProbeIrradiance` -- one number for the whole room -- and `GiBounces` 2 spends *a second ray per bounce ray* to move that guess one step back and still end on the probe. The hybrid replaces the second ray with a cone gather of the voxel grid at the first hit: the grid is a **cache of multi-bounce world-space radiance** (with `GiBounces` 2 it converges on every bounce, 4.4x the one-bounce number off screen against the traced second ray's 2.25x), so four rays a pixel buy a deeper answer than eight. **Cost NOT measured, and the owner reports ~90 ms a frame with it on (2026-08-19) -- see HANDOFF's high-priority item.** This row previously claimed 0.09 ms; that was the *grid build* and the gather was never timed. The hybrid calls a six-cone gather once per ray hit, four rays a pixel, at full resolution. **The grid is an input to the lit shader, not a writer of `Indirect`**, so the one-writer rule and the rays > voxel > screen order both stand. Needs: `RenderSettings::HybridSecondBounce`; the grid built when rays win (it is not, today); the radiance and face chains bound into the lit set with a params block, `voxel_cone.glsl` included unchanged over five accessor definitions -- which is why 8.1 factored it that way; and the editor's new voxel greying relaxed to `rays win AND not hybrid`. Band on `gi_away`: must beat the traced second ray's +1.83 while tracing four rays, ceiling under the grid's own 4.4x |
