...
|---|---|---|
| 8.3 | GPU-driven rendering | **Half done (2026-08-21).** The depth pass is GPU-driven: `DrawIndexedIndirect` exists on both backends and every depth view culls in compute. What is left is the lit pass — instance data, materials, probe selection and the depth sort all still per visible object on the CPU — and the meshlet path's second stage: the owner asked for meshlets on 2026-08-23 and the depth half landed the same day, flag-gated and bit-identical; the lit pass and task-shader feeding from the cull table are what remain. The depth half is kept because it is the scaffolding the lit half needs, not because of what it measured (7bx) |
