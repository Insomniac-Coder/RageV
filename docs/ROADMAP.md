...
| 7.10 | TAA — needs motion vectors first, which also buy motion blur and upscaling | L | 🚧 designed 2026-08-14 in ENGINE-NOTES §7r, not yet built. Three prerequisites — motion vectors (shared with 9.5, and worth landing and verifying alone), a jittered projection, and a history buffer the render graph cannot supply because its targets have no identity across frames. **The jitter must be indexed by frame number rather than by time**, or every screenshot check in `tools/scripts` stops being reproducible and the failure looks like noise. And the acceptance bar has a gap that has to be closed rather than lived with: a static scene lets TAA converge and flatters it maximally, while its only real failure mode is ghosting under motion |
