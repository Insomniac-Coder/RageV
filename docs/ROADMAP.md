...
|---|---|---|---|
| 8.1 | Global illumination — SDFGI or voxel GI | XL | **Still open, and not what 9.12 built.** 9.12 (2026-08-17) shipped indirect diffuse in the two forms that hang off machinery already here: a screen-space gather in the post profile, and four traced rays a pixel in the lit shader. Both are **one bounce, no cache, no probes**; the traced one is grainy until TAA settles and the screen-space one only knows what is on screen. What this row still means is the *world-space* form -- a signed-distance or voxel representation of the scene, multi-bounce, stable without a temporal filter, and lighting geometry that was never rasterised. That is the XL, and the reasoning below still applies to it: IBL plus good shadows plus 9.12 now gets most of the perceived benefit |
