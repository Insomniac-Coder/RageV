...
**Built, with one item outstanding.** The MSDF atlas, text, the canvas, sprite,
label and button, both particle paths and the GPU simulation all exist and the
camp's HUD uses them. **6.4's slider does not**, and is the only widget in that
list without a component — worth saying rather than leaving the table to imply
otherwise.

Added since: **`EmitterShape::Box`** (2026-08-21, 7bv). An emitter spawned every
particle at one point, and the only way to fake a volume — a full sphere of
spread and heavy speed jitter — does not work and cannot be tuned into working,
because the tell is the hole where the emitter is rather than the speed. Two
fields on the existing component rather than a second one: what was missing is
*where a particle is born*, and a second component would re-declare the colour,
size, curves, texture and blend that were already right. `ParticleMotion`
came with it, because "stationary" is not `Speed` at zero — gravity still takes
that one.
...
| 8.3 | GPU-driven rendering — compute-built draw commands, meshlets | XL | **Done 2026-08-21 except meshlets (ENGINE-NOTES 7bx).** `DrawIndexedIndirect` in the RHI on both backends; every depth view *and* the camera now cull in a compute pass against one object table, with the instance count read out of device memory rather than counted on the CPU. The lit pass reads its instance through a visible-index buffer the cull writes — the same indirection the CPU path fills, which is what makes one the other's fallback rather than its replacement. `--gpu-cull` and `--gpu-lit` switch each half. **Worth 4.35× on the owner's scene, 28 ms to 6.44**; on the scale fixtures 55 → 76 FPS at sixty thousand objects and 25 → 34 at a hundred and twenty. Matches the CPU path within that path's own run-to-run noise. Open half closed 2026-08-23: **meshlets** landed as a flag-gated Vulkan depth path (`--meshlets=on`, VK_EXT_mesh_shader, off by default) — a 64-vertex/124-triangle cut built per static mesh, drawn by a mesh stage that culls per meshlet against the light's clip volume, bit-identical to the vertex path on all three demo scenes and performance-neutral where the shadow phase is small. The lit stage followed the same evening -- pbr_meshlet.rvshader, the same ten-varying contract, bit-identical to the CPU vertex path and matching its cost -- leaving the task-stage/GPU-cull marriage; verifying it produced the first hard numbers on what was then called the gpu-lit defect (1.6M pixels apart on the showroom) -- resolved 2026-08-24, and the name was backwards: the CPU/meshlet submission was the broken side (EndScene's run reorder handed opaque runs to the OIT pass) and the gpu path was separately missing glass (blended indirect loop mis-strode SlotCommand rows); post-fix all four submission paths are bit-identical under tools/scripts/check_gpu_lit.py and the cost gap is gone (7.09-7.11 vs 6.88-7.08 ms interleaved). Two defects worth remembering are in 7bx: EndScene returned early on an empty pending list, so a scene of only static meshes drew the sky; and a raw pointer into Scene's slot table went stale whenever a reflection probe re-entered the render mid-frame, which read as a two-frame flicker and looked exactly like a frames-in-flight bug |
...
| 8.8 | XR | XL | 📦 **Archived 2026-08-20, the owner's call: not to be considered at all for now.** Kept as a row rather than deleted, the way 8.13 is, because the cost is worth having written down if it ever comes back: stereo rendering, a second projection path, and device SDKs -- and the last of those is the one that dates, since an XR runtime is a moving target in a way a file format is not. **Not a candidate. Do not propose it.** |
| 8.9 | FBX / Collada import | L | **Stage 1 done 2026-08-20 (ENGINE-NOTES 7bn): static geometry, materials, textures and the node hierarchy, via ufbx.** The row always said this was "a second material translation with its own failure modes", which is true, and implied a second *pipeline*, which reading the code disproved: `ImportedModel` was already format-neutral and **not one of its five consumers knows what parsed it**, so cooking, packing, the entity tree and the existing checks were all downstream of a struct this only had to fill. What the format actually costs is elsewhere -- axes and units (Maya is Y-up centimetres, Max is Z-up centimetres, Blender is Z-up metres, glTF is none of those), a material model that is Phong underneath, n-gon faces, and texture paths recorded from the artist's own machine. **Eight scenetest claims against a fixture written to both formats from one description**, and three failed on the first run: the unit one was the *fixture's* error and proved the importer right, and the material one was the importer fabricating a roughness from a shininess exponent -- the exact thing the design entry said it would not do, because ufbx fills its PBR view for Phong materials by deriving it. Gated on `shader_type` now. **Collada deferred on the owner's call** -- `.dae` is largely retired and glTF plus FBX cover the sources assets actually arrive from; picking it up later is a third front end against the same struct, which is now the cheap shape. **Stage 2 done 2026-08-22 (ENGINE-NOTES 7bz): skinning and animation, and the row is closed.** Three questions FBX answers differently from glTF, and only three. **The node transform is not a TRS triple** -- it is a ten-part chain with pivots, offsets and a pre-rotation, composed in an order the node itself names -- so clips are baked through `ufbx_bake_anim` rather than read off the `Lcl Rotation` curves. That is not a shortcut around reading them properly, it is the only faithful reading: `Rpre` is where Maya keeps joint orientation, so an importer that skips it rotates every joint by its joint orient, and euler angles interpolated between keys are not a slerp. **The rest pose comes from the bind matrices, not from the nodes** -- an FBX is a save file and is routinely written with the rig standing wherever the animator left it, while the bind matrix is the one thing in it defined to be the bind pose; deriving rest from it also makes "a skeleton at rest composes to the identity" true by construction. **The skeleton is what the skin binds**, not what carries a bone attribute, and a bone's parent is its nearest bone *ancestor* -- an FBX rig puts armatures and helpers between bones as a matter of course. **Fifteen scenetest claims against a twin fixture** that imports its geometry and its clip from make_skinned_gltf.py rather than restating them, so the two files cannot drift; the strongest samples both clips through the engine's own sampler at every sixteenth of their length and compares the composed matrices, and halving the fixture's bend was enough to make it fail. **Nothing downstream needed a line** -- the skinned vertex path, SkinnedBounds, the animator, the cooked format and the packager all worked the first time the FBX filled `ImportedModel`, which is stage 1's claim holding for stage 2 |
...
| 8.14 | The editor notices a file changing | S | ✅ **done 2026-08-21.** Not health work and therefore not phase 10, by that phase's own exclusion: it changes what the engine can do. **The evidence it came from is phase 10's shape, though.** Loaded assets were dropped only when a *project* was opened, so an editor left open while a mesh was re-exported kept showing the old one -- and reopening the scene did not help either, because scene loading does not clear them. Nothing said so; the only cure was a restart nobody had a reason to try. `AssetWatcher` polls the asset root with `<filesystem>`, which is portable where `ReadDirectoryChangesW`, `inotify` and `FSEvents` are three APIs; `Assets::Manager::Invalidate(path)` drops everything one source produced, **by path rather than handle** because a model file holds a handle per primitive and invalidating by the file's would reload the first and leave the rest. A change is reported one scan *after* it stops moving, since a tool writing a mesh puts it on disk long before it is finished. **And the cost was measured rather than assumed**, which is the part worth keeping: the first version scanned 340 files in **20.5 ms** -- a 25.7 ms spike in the editor's frame maximum -- almost all of it `fs::relative` per file, which normalises both paths and may touch the filesystem. A prefix strip gives the same answer for paths that came out of an iterator rooted at the root: **0.78 ms**, frame maximum 11.2, mean unmoved. A **permanent status bar** carries it, and exists because the editor had nowhere to say small things -- the dockspace is shortened rather than the bar overlaid, or it eats the bottom row of whatever is docked there | The two halves of one problem: the editor could not notice, and had nowhere to tell you if it had |
| 8.15 | The transform hierarchy walks whether or not anything moved | L | ✅ **done 2026-08-21 (ENGINE-NOTES 7bx).** `Scene::UpdateWorldTransforms` recomputed every entity's world matrix from its local one, four to seven times a frame, whether or not anything had moved — **27.4 ms of a 52 ms frame at sixty thousand objects**, more than everything else put together, and against ~0.6 ms for all the culling 8.3 is about. It now keeps a copy of the three vectors it last composed from and compares before composing; where they agree, and the parent's world did not move either, the existing matrix is already the answer. **Deliberately not the dirty flag this row proposed**, for the reason `Components.h` has always given: a flag has to be raised at every write site and a missed one leaves an object silently rendering where it used to be. Nothing reports a move here because nothing is trusted to — the walk looks. Still O(n), still cannot be got wrong, and the single largest win of the day: 18 → 33 FPS at sixty thousand objects on its own |
| 8.16 | Front-to-back sorting costs more than it saves past a few thousand objects | M | ✅ **done 2026-08-21 (ENGINE-NOTES 7bx).** Fixed in three steps, and the first two were wrong about why it was slow, which is the part worth keeping. The 256-byte instance payload came out of the sorted record, quartering it (33 → 45 FPS at sixty thousand). Sorting *indices* instead of records then bought almost nothing — 6.48 ms to 6.15 — because moving records was no longer the cost: **every comparison was**, dereferencing two indices into a four-megabyte array and missing cache twice. So pipeline, mesh, material and depth now pack into one 64-bit key and there is one sort over a flat array with no indirection at all, grouping falling out of the high bits and ordering out of the low ones. Splitting the timer found the rest: of the 4.12 ms left, the sort is 2.56, the permute 0.75, the key build 0.42 and the run ordering 0.38 — the permute was never the problem, which is what the first two attempts assumed |
| 8.11 | Asset store / plugin ecosystem | XL | ⏸ **Deferred 2026-08-20, the owner's call: "a large scale game engine thing".** Which is the right reading of it -- an asset store is not a feature, it is a distribution business with an engine attached, and the technical half alone needs a stable ABI that nothing here has or should have yet. Kept for the record; not a candidate while the engine is one person's. |
...
| 8.12 | Ray tracing — acceleration structures, ray queries, reflections and shadows | XL | **Done 2026-08-16, three stages the same day (ENGINE-NOTES 7am, 7an, 7ao):** acceleration structures in the RHI, ray queries proven in scenetest, and ray-traced shadows for **every** light kind behind one `RenderSettings::RayTracing` checkbox — hard-edged, bias-free, one ray per light per pixel, no cap on how many lights cast, skinned casters posed in compute and their structures refit each frame; checked against the maps to IoU 0.94–0.98 for the sun, a spot, a point light and the running fox. Then hit shading — buffers by address, a ray-instance table indexed by the instance's custom index, materials through the bindless heap — and with it **ray-traced reflections** in the lit shader where SSR's radiance enters (exact to 0.01 levels on 9.9's fixture and reflecting a block off the top of the frame that SSR cannot) and **ray-traced ambient occlusion** as SSAO's first pass (seam 15.17, brick wall at 1.000), each behind its own option under the checkbox, off by default, with the post profile's SSR/SSAO rows greyed and noted while the traced form runs. The whole block is offered only on a device with ray queries and under Shadows; OpenGL never shows it and takes the maps. Stated limits: hit shading is Lambert + emissive with a sun shadow ray only; rough surfaces keep the probe; no penumbra; RTAO is twelve rays through SSAO's blur, no accumulation. **Four defects and two costs, 2026-08-21, all in this row's code and none findable from a screenshot.** (a) *The editor lost the device in Play.* The ray-instance list was built from the terrain's **camera-dependent** level of detail -- an acceleration structure is world state and a LOD is view state, and the editor has two views. `Terrain::ForRays()` pins the ray level. 3x2500 frames clean where it had died within 400, 3 out of 3. (b) *RTAO strobed at exactly `TemporalJitterPhase`*: a position reconstructed from a jittered depth buffer is only unjittered if you subtract the jitter, and a screen-space pass cancels it through `ViewToUv` while a real ray cannot (7bq). Swing 1.48 % -> 0.21 %. (c) *The bounce was unbounded* -- `GiRadius` said "world metres a bounce may travel" and only the screen-space gather read it, so the traced form cast to 10 km: **3.2 ms a frame, zero pixels changed** when bounded. (d) *The half-resolution trace did not snap to the depth texel* it read, so on a surface seen at a grazing angle the ray origin sat centimetres off the plane against a 2 mm lift and the bounce rays hit the surface they left -- a lattice of dashes over the camper, reported as banding and Vulkan-only because OpenGL has no ray queries to run it (7bu). And the two costs: an acceleration-structure instance was **packed field by field into write-combined host memory**, which is a read-modify-write per bitfield over PCIe -- 2.96 -> 0.219 ms of CPU for one `memcpy` of a staged struct; and RT GI, RT reflections and AO now say **what they cost** rather than only whether they run (Off/Low/Medium/High, Off/Half/Full), which is 10.2's subject arriving from the other direction |

**The ordinary features are done.** 8.4 and 8.9 were the two, and 8.9 closed on
2026-08-22 with its stage 2. **Open, as of that date: 8.5, 8.6 and 8.7, all
XL** -- 8.3 closed on the 21st bar meshlets, which are not being started. Two
more rows are open and *not* candidates: 8.8 is archived and 8.11 deferred,
both on the owner's call, both kept as rows because the argument that put them
here is worth having if they return.

8.1 landed on 2026-08-19 and 8.10 on 2026-08-20; 8.13 was built on the 19th and
withdrawn on the 20th. 8.2 was a decision about the engine's identity, and it
has been made: the seam is at the shader define and the material, the RHI stays
one interface, and OpenGL is not dropped. 8.12 followed it and is done.

**So the phase has no ordinary feature left in it.** There is no next row here
in the sense the earlier phases had one -- what remains is a menu of major
commitments, and picking one is a decision about what the engine is for rather
than about what it is missing.
...
| 10.2 | Dead code and settings nothing reads | S | ✅ **done 2026-08-21.** Two settings the renderer ignored, both found while profiling rather than by auditing: `GiRadius` is documented as "world metres a bounce may travel" and only the screen-space gather ever read it -- the traced form cast to 10 km, which cost 3.2 ms a frame and changed no pixels when bounded. And `GiQuality` reached neither ray-traced form; RT GI and RT reflections are now Off/Low/Medium/High in their own right (7bs, 7bt) and AO is Off/Half/Full. The inspector rows that were merely *hidden* rather than wired were correct all along -- worth knowing, because this row assumes the lie is visible and one of these was not. **The sweep this row asked for was run on 2026-08-21 and came back clean**, which is the useful part: all 16 `RenderSettings` fields and all 47 `PostSettings` fields have a reader that is not the registry or the serializer; every `DisabledWhen` predicate reads live state and can be true; and no RHI entry point has only a test for a caller. Two candidates looked test-only and were not -- the grep had excluded `Platform/` and forgotten `RageVRuntime/`, which is worth saying because a sweep that reports dead code wrongly is worse than not running one. **What it did find was a whole target: `Sandbox` is deleted.** It targeted the renderer the RHI replaced, and five of the seven things it included -- `RageV.h`, `Entrypoint.h`, `Texture2D`, `FrameBuffer`, `OrthographicCameraController` -- no longer exist, so `RAGEV_BUILD_SANDBOX=ON` could not have succeeded. Ten files, a CMake option, a preset entry and four documentation rows describing a target that could not build. **Individual unused functions were left alone, at the owner's direction** -- `ShaderCompiler::SetCacheDirectory`, `EditorCamera::SetFOV`, `AudioEngine::SetVoicePitch`/`SetVoiceVolume`, `AssetManager::ReloadMaterial`/`ReloadPostProfile`, `ScriptGraph::RemoveLink`, `LayerStack::PopOverlay`, `RenderGraph::SetClearDepth` and `ImportCache::SizeOnDisk` all have no caller but are plausible API. `experiments/` is untouched: it is already an archive, not built and not on the include path, and its chunk generator is kept deliberately as the shape to avoid | Not a tidy-up: **a setting the engine ignores is a lie the inspector tells**, and the manual's drift check will happily document it. Wants a pass over `RenderSettings`/`PostSettings` for fields no resolve function reads, registry rows whose predicates can never be true, and RHI entry points with one caller that is a test. Evidence first — name each one and what reaches it |
| 10.3 | The costs never measured | M | **Done 2026-08-21.** The row asked for GPU timestamps that something quotes, a standing benchmark fixture and a recorded per-phase baseline. Passes now claim a GPU scope **by name**, summed and counted when one runs twice, so the editor's two graphs read as one row saying x2; 64 timestamp slots became 512. `--pass-timings=on`, implied by `--benchmark`. It paid for itself immediately and repeatedly: `scene/Scene` was 5.694 of 6.815 ms, which is where every finding below came from -- the unbounded bounce ray (3.2 ms), the acceleration-structure instance packed field-by-field over PCIe (2.96 -> 0.219 ms of CPU), and the bounce costing 2.87 ms inside the lit fragment against 1.43 ms outside it, same rays and same resolution, because a ray-query loop inherits the occupancy of the shader it sits in. Camp, editor, 2560x1494: **28.354 -> 6.16 ms/frame**, 35 -> 162 FPS, picture unchanged at every step. Also fixed the thing that made the profiler untrustworthy: it was handed `config.FrameTime` rather than the wall clock, so under `--frame-time` the "whole frame" row printed the flag's value while the phases inside it summed to more | 8.13's rule generalised: **a performance claim is a measurement or it is not a claim.** The profiler has GPU timestamps per phase and almost nothing quotes them. Wants a standing benchmark fixture and a recorded per-phase baseline, so a regression is visible as a number rather than as somebody noticing the editor feel slow. Note `--frame-time` makes the CPU frame column a constant and must never be passed to a benchmark |
| 10.4 | The claims that have never failed | S | ✅ **done 2026-08-20 as CHK.3 (ENGINE-NOTES 7bi).** Written against 7ba's two passes rather than over them, so the scope is the three places they could not reach: **what was written after CHK.2** (`check_graph` did not exist; `check_gi` grew the voxel claims), **a shape neither names** (a claim's evidence rather than its threshold), and **a break kind CHK.2 never used** -- every break for an effect switched it *off* or inverted it, so nothing had asked an effect to do too *much* except where a ceiling already existed to catch it, which is circular. Three findings, each measured before it was believed. **(1) `check_graph`'s "every graph generates" claim could not fail**: the generated C# is tracked in git and only the fixture's own file was deleted, so renaming a node type in `Roster.rvgraph` had the runtime log "at least one graph did not generate" in the same run the check printed OK. It compares content against git now. **(2) Two of the three lens effects had no magnitude at all** -- a vignette that ignores `VignetteSmoothness` and covers the whole frame passed, and so did a twelvefold aberration. The existing corner-versus-centre claim is blind to the first by construction: the corner reads 66.77 against 70.38 either way, and the whole difference is at mid-radius, 3.02 against 23.29. Five thresholds, each between a measured correct reading and a measured broken one. **(3) The aberration's fixture had nothing to disperse** -- the AA scene is two flat levels, so the effect reads 0.08 % of the frame and twelvefold only doubles it; on the tone ramp 9.3 built for the grain it goes 8.49 % to 58.71 %. **A fixture chosen for one effect cannot be assumed to measure another.** `falsify.py` gained a third target kind (a project asset, restored from git with its generated sibling) and four breaks. **Open, and stated rather than closed:** `require_drawn` is in 2 of 21 scripts where `require_current_shaders` is in 18 of 18 (two black frames are byte-identical, so every "off is off to the byte" claim is satisfied by a renderer drawing nothing); and the voxel claims are unaudited by anyone but their author -- `voxel-no-shadow` guards nothing on a fixture with no shadowed surface, and claim 15's two attempted breaks both survived and were deleted, all recorded in `falsify.py` before this pass and unmoved by it |
| 10.14 | Depth of field can follow a subject | M | ✅ **done 2026-08-22 (ENGINE-NOTES 7ce).** The owner's report: the car goes soft when the camera gets close to it. `FocusDistance` was a number in a file equal to the orbit's *starting* radius, so at the closest zoom the whole car sat in front of the near limit -- the scene's own comment had predicted the drift and shipped anyway. `Focus` is now `Manual | Target`; Target names an entity and solves **both** numbers from it every frame. The distance is the subject's depth along the view axis rather than the straight-line distance to it, because that is what the depth buffer the prepass reads holds. **The aperture is the half that makes it work**: putting the plane on a subject fixes where the sharpness is and not how much of it there is, and a 4.8 m car at 4.6 m does not fit in f/8 however well you focus -- so `N` is solved from the subject's own bounds against the 35 mm circle of confusion, in millimetres of sensor rather than pixels of output so a window resize is not a different photograph. `SubjectCoverage` spans the two looks a target can have, 1 for the whole subject sharp and 0.2 for a portrait. **At 7.3 m the solve returns f/8.12 and the scene had been hand-tuned to f/8 by eye** -- independent agreement to 1.5%. Almost none of the UI was new: `OnlyWhen`/`DisabledWhen`, `Enum` and the searchable combo all existed, and the entity slot's new type-to-search picker is inherited by every `EntityRef` field in the project. **The owner's correction closed the shared-profile case properly**: a target this scene does not have does not merely go unsolved, it reports `Manual`, because the same picture with a worse description is what leaves the next person debugging a mode the frame is not in. Following that through found the defect the picture could not show -- the inspector greyed `FocusDistance` and `Aperture` with "the focus target is answering this" *directly beneath a slot drawn in red as Missing entity*, switching off the two controls that would fix the shot. Whether a target resolves is a property of the profile **and the scene looking at it**, so it is `TargetResolved`, runtime state on `UIButtonComponent::Hovered`'s terms -- unregistered, unserialized, written by the solve for the renderer and by the profile drawer for the inspector. And the mode, the target and the coverage are all script-writable, which needed no plumbing because the render-settings bridge is registry-driven: a borrowing scene names its own subject in `OnCreate`. Proved by pointing the showroom's profile at a UUID nothing in the scene has and letting the camera script fix it -- **car sharpness 11.38, identical to the profile naming it directly, against 7.40 with no override**. **And the owner caught the half nothing could have caught**: the slot drew "Missing entity" in red against a perfectly good reference, above a dropdown with nothing in it to search. `DrawFields` -- the drawer every settings block goes through -- let `DrawField`'s `Scene*` take its default of null, which was right at every call site for as long as a settings block held nothing but numbers. The focus target is the first `EntityRef` ever to appear in one, and both halves of that drawer ask the scene. So it now has a fourth state: no scene at all is drawn disabled and says so, rather than making the specific claim "this id names nothing here" from a position where it cannot check. 27 scenetest claims |
| 10.6 | Transparent geometry | M | ✅ **done 2026-08-22 (ENGINE-NOTES 7cb).** The frame graph's transparent pass had existed since particles needed one and **nothing but particles ever fed it** -- every mesh was drawn opaque whatever its material said, and no scene had noticed because no scene had glass in it. glTF's `alphaMode` and FBX's `opacity`/`transparency_factor` now reach the draw list; the blend mode lives on the Material as a *routing* decision, and the transparent variant is pbr.rvshader with `RV_TRANSPARENT` -- the same lighting into two attachments instead of four. One bit at the top of the sort key puts the blended draws in a contiguous block at the end, so the opaque loop stops where it begins. (The 7.8 run reorder defeated that partition from the day this shipped until 2026-08-24 -- it re-sorted runs by nearest member with no transparent tier, handing every opaque run farther than the nearest glass to the OIT pass; the reorder now compares the transparent bit first, and check_gpu_lit.py holds the line.) **Coverage is not transmission**: a pane at alpha 0.2 transmits four fifths and still reflects the ceiling at full strength, so the two terms are separated and recombined as `transmitted * alpha + reflected` with coverage `alpha + reflectance * (1 - alpha)`. Blended meshes are also out of the shadow maps and out of the ray-tracing structure, because every ray treats that structure as opaque and glass that shadows like a wall leaves the cabin and every lamp interior unlit. **Also on the GPU path (7cb):** GpuCull holds two tables, the blended one culled beside the opaque one and issued during the transparent pass, with the CPU list as the fallback bindless-less devices take. (Its indirect loop stepped 20-byte offsets over the cull's 24-byte SlotCommand rows until 2026-08-24, so only the first blended mesh ever drew from it; every indirect consumer now strides by sizeof(GpuCull::SlotCommand)) |
| 10.5 | Minor improvements found in passing | S | The catch-all, and deliberately the lowest priority here. Anything small, evidenced and unblocking: a confusing tooltip, a log line that says nothing, an editor row in the wrong section. **Not a licence to refactor** — if it needs a design, it needs a row of its own. **Collected 2026-08-21, all reported by the owner rather than found by looking:** the viewport wheel had a *well* — the zoom step was metres proportional to the **square** of the distance, so at the one-metre floor a notch moved six millimetres and climbing back out was equally slow, 142 notches in and 147 to return; it is a ratio now, 22 each way and symmetric. The fly throttle floored at 0.05 m/s, two flicks of the wheel from the default, and at that speed a second of W moves five centimetres — indistinguishable from the key doing nothing, and so indistinguishable from S doing nothing, which is exactly how it was reported. And **W was firing twice**, as fly-forward and as the translate gizmo, so the gizmo keys now yield to the camera while the right button is down. Also: the camp's cinematic camera lerped between shot markers, so its travel was a straight line through the van — a constraint on the states is not a constraint on the transitions (7bw) |
| 10.9 | Arrays and dictionaries | S | ✅ **done 2026-08-20 (ENGINE-NOTES 7bh).** The last of 7bh's three exclusions, and it did not hold either: a pin *can* say what it iterates, as long as the element type is written down once rather than invented per instance. Four pin types — `NumberList`, `EntityList`, `NumberMap`, `EntityMap` — and 38 nodes. Maps are string-keyed always; a second key axis would double an already-doubled set. `For Each` is a `for` over `Count` rather than a C# `foreach`, because the node offers an Index and `foreach` has none. The scope rule generalised over the whole loop family rather than being copied per type. **137 nodes.** Also gives the content browser's back button a chevron icon instead of a `"<"` |
| 10.8 | Loops and user-defined functions | S | ✅ **done 2026-08-20 (ENGINE-NOTES 7bh).** For Loop, While Loop, Break, Function and Call. **7bh's reason for excluding loops was wrong**: `Branch` already emits a nested block, so a loop is that shape plus a `Completed` continuation, and the scope a loop needs *is* expressible — it is what the Body output reaches. The validator computes it and enforces two rules: Index may only be read inside the Body, and Break must be inside a loop. While Loop carries a runaway guard that warns and breaks at a million iterations, because an unbounded `while` in a script freezes the editor with no clue why. Functions are parameterless by design — pins come from a node type, so a per-instance signature needs dynamic pins the descriptor model does not have, and the graph's variables are already class fields a caller can set. Also fixes two fixtures left naming a removed node type by an uncommitted rename, and makes check_graph assert that *every* graph in the project generates |
...
| 10.10 | A graph silently drops what it cannot read | S | ✅ **done 2026-08-20 (ENGINE-NOTES 7bj).** `ScriptGraphSerializer::Load` dropped a node of unknown type and returned success, so a `.rvgraph` naming a type this build no longer has generated **silently emptied** -- `Roster.g.cs` thirty-one lines to an empty `OnCreate`, and `dotnet build` compiled it. A partial graph is now the one answer the loader cannot give: an unknown type is an error, in the shape the version check above it already used, and the version field keeps meaning a *format* change. **Two things had to move with it or the refusal only relocates the loss**: `GenerateAll` removes the stale `.g.cs` (the rule `GenerateToFile` already had for a graph that fails to validate, one step earlier), and `Manager::GetScriptGraph` returns null instead of an empty graph -- its header used to promise "never null for a valid handle", which is a blank canvas over a file with contents and a first Save that makes it true. `s_FontFailed` had the right shape all along. **And the owner caught the half that was missing**: refusing leaves the user nowhere, since the graph cannot be repaired in the tool that made it. The data loss was never *loading* a graph with an unknown node -- it was doing it silently, automatically, at generate time. So the refusal page has a second button, **Open without it**: it names what goes ("1 node ('NumbersAppend') and 4 links that touched it"), opens **dirty** so the asterisk and the unsaved-changes guard apply, keeps a banner up until the graph is saved, and leaves the file untouched until then. `GraphLoadMode::DropUnknown` is what it asks for; `Strict` stays the default for every caller running without somebody watching. The validator then lists what needs rewiring for free -- nine problems on the real fixture. **Eight scenetest claims**, falsified by putting the old drop back (two go red). `--graph-drop-unknown=on` drives the recovery path for a screenshot and calls the button's own handler rather than a copy of it |
| 10.11 | The one asset the editor cannot create | S | ✅ **done 2026-08-20 (ENGINE-NOTES 7bk).** Found while writing the manual's visual-scripting page: every other asset type is created from the field that needs it -- "New post profile...", "New colour LUT...", "New Script..." -- and a `.rvgraph` had no such entry anywhere. It could be opened, saved and generated from, but not started, and `Manager::CreateScriptGraph` had been written for it and had **no callers**, which is also 10.2's subject. **New Graph...** now sits beside **New Script...** in the same dropdown, for the reason that comment already gave: choosing what runs here and making a new one are the same decision. C# only, because C# is what a graph generates. It writes a starter -- On Create -> Log with the text on a literal, **one execution wire and one data wire**, which is the smallest graph that shows both kinds and the one thing about a graph that cannot be guessed -- refreshes the registry, selects the name, and opens the canvas through the editor's own activate handler rather than a second route to it. **The starter lives on `ScriptGraph::Starter` rather than in the panel**: it is content, and content nothing headless can reach is content nothing can check. Five scenetest claims cover it -- it validates clean, it generates, into the event the canvas shows, the literal's text reaches the statement, and it has exactly one wire of each kind |
| 10.12 | Graph was hiding inside C# | S | ✅ **done 2026-08-20 (ENGINE-NOTES 7bl), the owner's correction to 10.11.** New Graph... had been put at the bottom of the C# script dropdown, reasoning that a graph generates C# so that is where it belongs. **That let the implementation's structure become the user's menu**: C# is how a graph is executed, not what the person chose, and finding the feature required knowing how it works -- the exact leak 7bh's design exists to avoid, with the manual page arguing the opposite three paragraphs in. Language is now three-way (C++, C#, Graph); picking Graph asks for a name straight away, the picker below lists `.rvgraph` assets rather than every class in the assembly, and a graph-backed component gets **Edit Graph**. **The kind is the file system, not a flag**: `assets/graphs/<Name>.rvgraph` existing is the fact, so there is no scene format change, no migration and no second source to arbitrate when a graph is deleted outside the editor. Found on the way: picking Graph from a **C++** component queues the conversion first, so the popup request has to survive into the next frame -- the first cut consumed it in the native picker, which never calls BeginPopupModal, and the component converted with no dialog ever appearing |
| 10.13 | A graph variable can say what it is | M | ✅ **done 2026-08-20 (ENGINE-NOTES 7bm).** The owner's ask: public and private sections for variables and user-defined functions, and a ShowInEditor checkbox. **The clause that came with it -- "changing a public variable on one object should not affect another object using the same graph" -- turned out to be a question, and answering it first is what kept this small**: a graph generates a class, two entities are two instances, and `ScriptFieldOverrides` lives on the component, so per-entity values were never at risk and `public` does not change that. Nothing was built for it; the manual says it now. Two flags because they are two questions -- `Public` is the field's access modifier, `ShowInEditor` is whether the Script component draws a row -- and the useful combinations are not diagonal: public-and-hidden is something another script drives, private-and-shown is the ordinary tuning number. **ShowInEditor needed a mechanism that did not exist**: `ScriptFields.Editable` reflects every instance field, private included, on the deliberate argument that opting *in* is a concept nobody should need -- so ScriptCore gains `[HideInEditor]` as the opt-out, and hand-written C# gets it too. **Declarations are additive and that is checked, not hoped**: the serializer writes one only when it says something, and `check_graph` confirms every committed `.g.cs` regenerates byte-identically. Variables are *listed* from the nodes rather than created in the panel -- a name exists because a node carries it, and an add button would make undeclared-but-used and declared-but-unused into questions with no good answers. Six scenetest claims |
...
*Checked 2026-08-21. Four of the five below are closed, and the list is kept
rather than deleted because what a first pass found is worth being able to
compare against.*

- ~~**`NativeScriptComponent::Bind` captures `this` by reference.**~~ **Gone.**
  `OnInstantiateFunction` and `DestroyInstanceFunction` no longer exist; nothing
  in the tree names them, so the dangling capture went with the design.
- ~~**`OnDestroyFunction` and `DestroyInstanceFunction` are never called.**~~
  **Fixed.** `OnDestroy()` is invoked from two sites.
...
*Twice replaced, and this is the second. The original — Phase 0 completely,
then Phase 1, Phase 3 pulled forward only for a visible milestone — held. Its
replacement said "do Phase 5 next, then build a game with it, and if a phase
must be picked instead, Phase 6". **Phase 5 is done and so is Phase 6.** Half of
...
|---|---|---|
| 8.3 | GPU-driven rendering | **Half done (2026-08-21).** The depth pass is GPU-driven: `DrawIndexedIndirect` exists on both backends and every depth view culls in compute. What is left is the lit pass — instance data, materials, probe selection and the depth sort all still per visible object on the CPU (the indirect path draws in the cull's atomicAdd order and never consults --depth-sort: measured 6.3 ms on the pathological overdraw fixture where the sorted CPU path pays 1.17, 2026-08-24 — the number a GPU-side ordering would chase) — and the meshlet path's second stage: the owner asked for meshlets on 2026-08-23 and the depth half landed the same day, flag-gated and bit-identical; the lit pass and task-shader feeding from the cull table are what remain. The depth half is kept because it is the scaffolding the lit half needs, not because of what it measured (7bx) |
...
than by being designed, every one of them silent. 2026-08-21 was a day of
nothing but that: a camera that flew through the van because a constraint on the
shots was never a constraint on the travel between them; a particle emitter that
could only spawn at a point; a viewport wheel with a well in it; an editor that
showed a stale mesh with nothing to say so. Four capabilities and four defects,
**none of which had a roadmap row before the day started**, and all of which were
in somebody's way.

The one thing on the list that would earn its place first: **10.5 has become the
place this arrives**, and 10.2's sweep is the only outstanding row with a
concrete deliverable behind it.

If a phase must be picked instead: there is no longer an obvious one. The six
remaining Phase 8 items are each a multi-week commitment to a direction — GPU
driven rendering, navigation, networking, a second platform, XR, a plugin
ecosystem — and choosing between them from the inside is exactly the guess this
