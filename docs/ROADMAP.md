...
every other open row is XL. **Open and under consideration, as of 2026-08-21:
8.5, 8.6, 8.7 and 8.9** -- 8.3 closed on the 21st bar meshlets, which are not
being started -- and 8.9 is the only one of those that is not an
engine-sized subsystem on its own. Two more rows are open and *not* candidates:
8.8 is archived and 8.11 deferred, both on the owner's call, both kept as rows
because the argument that put them here is worth having if they return.
...
**So the phase has one ordinary feature left in it.** After 8.9 there is no
next row here in the sense the earlier phases had one -- what remains is a menu
of major commitments, and picking one is a decision about what the engine is
for rather than about what it is missing.
