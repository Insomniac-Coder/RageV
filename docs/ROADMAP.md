...
|---|---|---|
| 8.3 | GPU-driven rendering | **Half done (2026-08-21).** The depth pass is GPU-driven: `DrawIndexedIndirect` exists on both backends and every depth view culls in compute. What is left is the lit pass — instance data, materials, probe selection and the depth sort all still per visible object on the CPU — and meshlets, which nothing has asked for. The depth half is kept because it is the scaffolding the lit half needs, not because of what it measured (7bx) |
