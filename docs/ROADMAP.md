...
| 10.2 | Dead code and settings nothing reads | S | **Part done 2026-08-21.** Two settings the renderer ignored, both found while profiling rather than by auditing: `GiRadius` is documented as "world metres a bounce may travel" and only the screen-space gather ever read it -- the traced form cast to 10 km, which cost 3.2 ms a frame and changed no pixels when bounded. And `GiQuality` reached neither ray-traced form; RT GI and RT reflections are now Off/Low/Medium/High in their own right (7bs, 7bt) and AO is Off/Half/Full. The inspector rows that were merely *hidden* rather than wired were correct all along -- worth knowing, because this row assumes the lie is visible and one of these was not. Still wanted: the sweep this row actually asks for, over every RenderSettings/PostSettings field and every RHI entry point whose only caller is a test | Not a tidy-up: **a setting the engine ignores is a lie the inspector tells**, and the manual's drift check will happily document it. Wants a pass over `RenderSettings`/`PostSettings` for fields no resolve function reads, registry rows whose predicates can never be true, and RHI entry points with one caller that is a test. Evidence first — name each one and what reaches it |
