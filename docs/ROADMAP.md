...
| 7.13 | MSAA | L | 🚧 groundwork done 2026-08-14, **mode switched off**. The RHI is complete on both backends — multisampled targets, hardware resolve, and `GetColorTexture` handing out the resolve so the frame graph never learns MSAA exists — and it measures **0.1912 → 0.0597 at 43°**, beating SSAA 2× at the same sample count, with zero validation lines. `FrameGraphBuilder` refuses it because of one unresolved crash: six entities (camera, light, reflection probe, particle emitter) segfault on frame one, on both backends, with validation silent. Probe capture re-renders the scene into a target of a different shape, and every renderer holds one pipeline set for one shape. Next step is a pipeline cache keyed on target shape, not more guessing. ENGINE-NOTES §7q | The real RHI item: sample counts on textures, render targets and pipelines, plus a resolve attachment and the multisampled depth the scene pass needs. The OIT accumulation attachments share the scene target, so they have to move with it. Unlike the post filters it resolves in *linear* space, before the tone curve, which is the correct place and a visible difference on high-contrast edges |
