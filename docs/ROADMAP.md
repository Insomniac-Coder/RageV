...
| 3.7 | Skeletal animation — skinning, clips, blending | XL | 🟡 three of five pieces: skeleton/clips/sampling/blending (CPU, tested), shader includes so the skinned variant shares the PBR lighting, and glTF skin+animation import against a generated two-bone asset. Left: skinned vertex format, pbr_skinned.rvshader, the renderer and shadow paths, and the components. **Nothing renders skinned yet** |
