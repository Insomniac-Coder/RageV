...
| 10.10 | A graph silently drops what it cannot read | S | **Found by CHK.3 (ENGINE-NOTES 7bi, finding 2), open.** `ScriptGraphSerializer::Load` drops a node of unknown type and returns success, so a `.rvgraph` naming a node type that no longer exists does not fail to generate -- it generates **silently emptied**. Measured: `Roster.g.cs` went from thirty-one lines to an `OnCreate` with nothing in it, and `dotnet build` compiled it. That is exactly what `check_graph`'s claim 2 forbids in its own words -- *an empty class compiles, attaches, runs and does nothing* -- enforced until now only for the one fixture named `Broken`. **This is data loss in an authoring surface**, not a check problem: the check was fixed to compare against what is committed and catches it on the fixtures, and nothing catches it on a user's graph. The fix is for the loader to refuse the graph rather than drop the node, which means deciding what the editor shows when a project is opened against an engine that has removed a node type -- a migration question, which is why it is a row and not a patch |
