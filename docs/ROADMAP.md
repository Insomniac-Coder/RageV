...
| 8.3 | GPU-driven rendering — compute-built draw commands, meshlets | XL | **Depth pass done 2026-08-21 (ENGINE-NOTES 7bx); the rest open.** `DrawIndexedIndirect` in the RHI on both backends, and every depth view — cascade, spot, cube face — now culls in a compute pass against a table of every static object, with the instance count read out of device memory rather than counted on the CPU. `--gpu-cull=on\|off` compares the two paths on one binary. **Worth 4% of the frame at 20,000 objects and under 2% at 60,000**, because the walk it removes is 24 bytes an object and the table it uploads is 96. Open: the lit pass, which is where the per-object cost actually lives (16 ms of the 60,000-object frame), and meshlets. **And the measurement found the real wall elsewhere: `UpdateWorldTransforms` is 27.4 ms of that 52 ms frame across seven calls, against ~0.6 ms for all the culling put together** — see 8.15 |
...
| 8.15 | The transform hierarchy walks whether or not anything moved | L | **Found by measuring 8.3 (ENGINE-NOTES 7bx), not yet started.** `Scene::UpdateWorldTransforms` recomputes every entity's world matrix from its local one, from the root down, with no notion of what changed. At 60,000 objects that is **27.4 ms of a 52 ms frame -- more than everything else in the frame put together**, and against ~0.6 ms for all the frustum culling that 8.3 is about. **And the call count is not fixed:** the fixed-step loop walks the hierarchy after every `OnTick`, so a frame slow enough to run three steps pays three extra walks -- seven calls a frame at 60,000 objects against four at 20,000, which means the cost rises with the frame time it is causing. The fix is dirty tracking, and the reason it is L rather than M is stated in `Scene.h` about the draw list's own flag: *a flag that has to be set at every write site is a flag one of them will forget*, and a missed one is an object frozen at last frame's transform. So it needs the write sites narrowed before the flag is worth having, which is a change to how transforms are edited rather than to how they are walked. Nothing until a scene is large enough to notice, and then it is the whole frame |
| 8.16 | Front-to-back sorting costs more than it saves past a few thousand objects | M | **Measured 2026-08-21, not yet fixed.** `--depth-sort=on|off` on one binary: the sort spends 0.07 / 0.72 / 3.04 / 12.22 ms of CPU at 1,000 / 5,000 / 20,000 / 60,000 objects and saves 0.00 / 0.00 / 0.04 / 0.59 ms of GPU. **A net loss at every size measured**, and at 60,000 it is 40% of the frame. The technique is not wrong -- this engine's own note records it taking a heavily-overlapping test scene from 33.9 ms to 0.32 -- but that was measured at *200 objects*, and the same shape as 8.3's original premise: **its cost scales with object count and its benefit scales with overdraw, and those part company**. The implementation is what is expensive: `std::sort` over `PendingDraw`, which carries 256 bytes of instance data inline, then a full copy through scratch and back -- about 20 MB shuffled at 60,000. Sorting `(float depth, uint32 index)` pairs and gathering once does the same reordering for a fortieth of the traffic and keeps the overdraw win. The two real scenes are unaffected either way (camp 4.96 against 4.94 ms, demo 2.72 against 2.73 at 1080p): both are GPU-bound and few-object, which is the other half of the finding |
...
every other open row is XL. **Open and under consideration, as of 2026-08-20:
8.3, 8.5, 8.6, 8.7 and 8.9** -- and 8.9 is the only one of those that is not an
