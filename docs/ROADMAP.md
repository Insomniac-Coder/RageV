...
| 7.10 | TAA — needs motion vectors first, which also buy motion blur and upscaling | L | 🚧 designed 2026-08-14 in ENGINE-NOTES §7r; **two of the three prerequisites built**, the history buffer is not. Motion vectors are done and verified alone, before any filter existed to hide whether they were right — every instance carries its previous world transform and the scene target has an RG16F velocity attachment, which 9.5 also wants. The jitter is done: a centred Halton(2,3) point applied to the scene camera in one place, so meshes, sky, grid, world text and particles all move together, and **indexed by frame number rather than by time** — a clock there would make every screenshot check in `tools/scripts` irreproducible with a failure that looks like noise. `check_taa_jitter.py` measures that directly: same frame twice is byte-identical, frame 30 and frame 38 are byte-identical to *each other* while 30 and 31 differ (a clock can be reproducible and still have no period), all eight offsets distinct, and nothing more than four pixels from an edge moves at all. Selecting TAA today gets the jitter and no accumulation, which is worse than None and says so in the inspector. **And the history is in too.** `TemporalHistory` is a caller-owned ping-pong pair — the graph cannot supply it, because its targets are pooled by shape and have no identity across frames — imported back in, with the target written this frame serving as both the rest of the chain's input and next frame's history, so nothing is copied. The resolve runs on the linear HDR scene in the SSAA slot, which puts TAA on the *supersampling* side of §7n's two definitions of correct. Rejection is a YCoCg neighbourhood **clip** rather than a per-channel clamp, plus a 1/(1+luma) weighted mean. On a static scene it measures **the best of all six modes at three of the four angles** on the linear yardstick — 0.0444 at 47° against SSAA's 0.0610 and MSAA's 0.0609, 5.1× better than no filter at 77° — both backends agreeing to four decimals. That number flatters it and the notes say so: a static scene is TAA at its best. **It is not finished, by §7r's own standard, written before any of it was built:** *"Its only real failure mode is motion... That check does not exist yet and TAA is not finished until it does."* Nothing in this repository has yet rendered TAA with anything moving, so the case it is *bad* at is entirely unmeasured, and two claims rest on argument rather than evidence: the **sky reports no motion** so it smears when the camera turns (the fix needs a uniform buffer — the sky's push-constant block is 112 of a guaranteed 128 bytes and a previous view-projection is 64 more), and the **reprojection's vertical sign** is argued from how every other fullscreen pass behaves, which a still frame cannot confirm because velocity is zero everywhere in one. **Remaining:** a deterministic moving scene — a rigid body under the fixed step is reproducible — a reference to compare it against, and the two fixes it would then be able to judge |
