...
| 10.11 | The one asset the editor cannot create | S | **Found while writing the manual's visual-scripting page (2026-08-20), open.** Every other asset type is created from the field that needs it -- "New post profile..." in the profile dropdown, "New colour LUT...", "New Script..." in the Script component's. A `.rvgraph` has no such entry anywhere: it can be opened, saved and generated from, but not started. `Manager::CreateScriptGraph` was written for it and **has no callers** -- which is also 10.2's subject, an API reachable only by a test. The manual currently has to tell people to copy a file in their file browser and press Refresh, which is a documented workaround for a missing button. The natural home is the Script component's dropdown beside "New Script...", since a graph is a way of authoring the same thing |
