...
| `--hybrid-gi=on\|off` | Force `HybridSecondBounce` for this run: where ray-traced GI runs, the first hit is shaded from the voxel grid instead of the probe and no second ray is traced. Needs ray-traced GI and a device that can voxelise; says so once and stays off otherwise. ENGINE-NOTES 7be |
| `--render-defaults=on\|off` | Ignore the project's Render Settings and use the struct's defaults for this run; the other overrides here still apply on top. What every check script passes, so a run depends only on its own command line and never on what the editor last saved. Never something the editor should be given. **Required whenever `--screenshot` is given** -- the runtime refuses to start otherwise, because a measurement that silently inherits the project's saved settings is not reproducible: a saved `RayTracing: true` selects a different form of GI, SSR and AO, and the run measures one thing while its author believes it measured another. Say `off` to screenshot a project as it is actually configured. ENGINE-NOTES 7ba |
...
| `--screenshot-count=N` | Capture N consecutive frames from that one, in one run, as `<file>_<frame>.png`, then exit. Default 1 (the file as named). The way to look at a flicker: separate runs are separate clocks. |
