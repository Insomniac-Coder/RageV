...
| `GiBounces` | `1` | 1 or 2 | ray-traced global illumination |
...
| `GiBounces` | `1` | How many times traced light bounces before the reflection probe answers for the rest. At 1 every bounce ray's hit is shaded with the probe's guess at what reaches it, which in a room the probe does not describe well leaves anything that can see nothing directly lit as dark as that guess. At 2 one more ray answers instead, and *its* hit takes the probe -- so the path ends at depth two by construction. Only read while ray-traced global illumination is on: a screen-space gather has one bounce and no way to have two. ENGINE-NOTES 7ax | One more ray per bounce ray. The temporal filter absorbs the noise; the rays still cost what rays cost |
