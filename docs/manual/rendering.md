...
| `HybridSecondBounce` | `false` | — | where ray-traced GI runs: shade the first hit from the voxel grid instead of the probe, and trace no second ray |
| `VoxelGlobalIllumination` | `false` | — | where a profile's Global illumination is on, and rays do not win; **greyed in the editor while ray-traced GI is on**, because the grid is not built at all then -- unless `HybridSecondBounce` is on and reads it |
...
| `VoxelGlobalIllumination` | `false` | Where a camera's profile asks for global illumination, gather it from a **voxelised scene** instead of from the screen: every frame the scene is rasterised into a clipmap of 3D grids around the camera, lit from the shadow cascades and the local lights, mipped per viewing direction, and cone-traced from every pixel. Light arrives from behind the camera, behind other things and off every edge of the frame -- what the screen gather cannot know -- **on both backends, with no ray hardware**. The profile's Global illumination stays the on switch and its GI intensity, quality and denoise apply; its GI radius does not (a cone runs to the grid's edge) and its row says so. **`HybridSecondBounce`** (8.13, ENGINE-NOTES 7be) is the one setting in which the voxel grid and ray tracing cooperate rather than exclude each other. With ray-traced GI on, the lit shader shades each ray's first hit from a cone gather of the grid instead of the reflection probe's single number, and traces **no second ray** for it. Measured on `gi_away`, off screen: one traced bounce +0.81, a second traced ray +1.83, the hybrid **+1.47** -- about **four fifths of a second bounce for half the rays**, plus roughly 0.1 ms to build the grid. It is a trade rather than a free win: worth it where the frame is ray-bound, not otherwise. It builds the grid even though the voxel *gather* never runs beside rays, so `VoxelGiResolution`, `VoxelGiCascades` and `VoxelGiVoxelSize` apply and stay editable. Vulkan only in practice, since it needs ray queries.
