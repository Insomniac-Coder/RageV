...
| **Show the build in File Explorer when it finishes** | Opens the output folder once the build has succeeded, with the game's executable selected in it. Off by default and **remembered across restarts**, in `ragev.ini` beside the editor -- it is a preference about how you work rather than anything about the project. A build runs on a worker, which is why it is not on by default: the window would otherwise arrive minutes later, over whatever you had moved on to |
...
| Build Game's "show the build" checkbox | The moment you tick it |
