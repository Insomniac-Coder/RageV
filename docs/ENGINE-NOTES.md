...
**So the falsify lever works on Vulkan and CHK.2's guarantee stands.** What is
left is much narrower and still open: **on `gi_shadowed` specifically, and in
the floor strip this entry measured, Vulkan reads +0.50 and does not respond
to the injection at all** -- reproducibly, and not an artefact of which backend
runs first (running OpenGL first and Vulkan twice gives +0.07, +0.50, +0.50).
The same build on `gi_corner` responds normally. So it is a property of that
fixture and that region on that backend, not of how shaders are loaded, and
whoever writes a claim on `gi_shadowed` has to explain it before trusting a
number from there.

One more thing the corner test showed that is worth a look on its own: the two
backends respond to the *same* quarter-scale edit by different factors --
Vulkan 1.78 to 0.38 (0.21), OpenGL 1.83 to 1.10 (0.60). Both moved, so neither
is stale, but they do not agree about how much of the bounce comes from the
injected level against the mip chain built over it.

**The local-light shadow follow-up is therefore not blocked** by anything
found here; it was left unbuilt for time, not for tooling.
