...
**DESIGNED 2026-08-17, NOT YET BUILT.** The session ran out of budget after
the design; the code below this entry describes is the next thing to write.
Draft patch scripts for `TerrainBrush.h`/`.cpp` (the mask, the noise, the
shape/pattern weights, and the four operations, exactly as designed here)
were parked in `build/7as-drafts/` (git-ignored, on the owner's machine);
apply `brush_h.py` then `brush_cpp.py` from the repo root, build, then do
the tool, the inspector, `--brush` options, `make_brushes.py`, the `stamp`
fixture and the checks. Nothing in this entry has been verified yet.
...
**The masks are files.** `RageVEditor/assets/brushes/*.png`, eight of them
generated by `tools/scripts/make_brushes.py` (numpy, seeded, 128 x 128,
committed): `square`, `diamond`, `star`, `ring`, `cloud` (fBm, soft),
`splatter` (thresholded noise blobs inside a soft disc), `streaks`
(directional, the one that follows the stroke), `stripes` (half white, half
black -- a pattern for the check, and a usable one), and `dots`. The editor
...
sun): the *editor* paints layer 1 with `shape=square` at angle 0 and the
pixel at (0.9 R, 0.9 R) -- inside the square, outside the disc -- reads
blue; at `angle=45` the same pixel reads red (the corner has turned away)
while (0, 0.9 R) is still blue; with the disc it reads red; and a paint
through `pattern=stripes` at `scale=16` puts blue at x = 4 and red at
x = 12 -- the stripe and the gap, derived from the tiling. Falsified by
skipping the rotation (45 degrees reads as 0), and by sampling the pattern
in brush space instead of terrain space (the stripes move with the stroke
and the x = 12 pixel goes blue).
