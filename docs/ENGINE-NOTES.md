...
### A fifth finding: the falsify lever does not reach Vulkan on this shader
...
Read the last two rows. On OpenGL the injection responds exactly as it should:
kill the shadow and the bounce vanishes, quarter the radiance and the bounce
quarters. **On Vulkan nothing moves at all, to two decimal places, under any
of them.** A change that cannot fail to alter the picture -- multiplying every
injected voxel's radiance by 0.25 -- leaves the Vulkan frame identical. So the
Vulkan path is not running the shader on disk; it is serving something
compiled earlier.

**What this costs is bigger than the shadow term.** `falsify.py` works by
patching deployed shaders precisely because that needs no rebuild, and CHK.2's
guarantee is that every claim in every check script has been shown to fail
when the thing it measures is broken. **That guarantee does not hold for
`voxel_inject.rvshader` on Vulkan**, and it is not yet known how far the
problem extends -- `voxel-no-lift`, which edits `include/voxel_cone.glsl`, did
move Vulkan's numbers, but the gather includes that file too, so the change
seen may have come entirely from the gather and not from the injection.

Whoever picks this up: the question is what invalidates the compiled-shader
cache on the Vulkan path and why an edit to a compute `.rvshader` misses it,
starting from `ShaderCompiler` and the cache hash 8.2b added for
`ShaderDesc::Defines`. **Until it is answered, no falsify break that lives in
a compute shader can be trusted on Vulkan, and the local-light shadow
follow-up cannot be verified there** -- which is why that follow-up is not
built: it would be unverifiable on the backend it matters most on.
