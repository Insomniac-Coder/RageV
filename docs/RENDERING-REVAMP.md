...
#### R2 · Point the benchmark's per-pass counters at the renamed passes — ✅ done 2026-09-06 night

> **Result.** The graph prints its passes as `scene/<Pass>`; the pattern now accepts the prefix, `ReflectionBlur` is a column, the dead `unscaled` arm is gone (four A B B A pairs of `scaled` / `off`). Two runs of 200 frames at the owner's camera, 1600x900: frame 10.78 / 10.80 ms, trace 2.38 / 2.40, resolve 1.53 / 1.54, accumulate 0.27, blur 0.09, composite 0.03, scene 5.10; spread within an arm 0.04 ms. The reflection passes are 4.3 ms of the 10.8; scene 5.1; the rest (transparent 0.57, SSAO 0.25, TAA 0.12, tonemap/bloom/budget ~0.3) accounts for the remainder to within 0.1 ms. The in-line `off` arm is 25.8 ms. **Folded in (owner):** the `rays per frame: reflection` counter read 0.04 M because the trace pass never flushed its tally (the lit shader's flush is compiled out under `RV_TRACE_ONLY`) and, had it flushed, its rays would have landed in the GI lane (`RAY_LANE_SURFACE` follows `RV_TRACE_ONLY`). `reflection_trace.rvshader` now defines `RV_REFLECTION_TRACE`, which `pbr_fragment.glsl`'s lane chain maps to the reflection lane, and calls `FlushRayCounters(false)` after its ray. Reads **1.40 M reflection rays a frame** at 1600x900 (1.44 M pixels: one ray a glossy texel), and the shadow lane rose 18.6 -> 25.3 M: the hits' shadow rays, never counted before. Frame unchanged, 10.76 ms.
...
#### R3 · The preset's MirrorRays column sets the gathered neighbours — ✅ done 2026-09-06 night

> **Result.** `MirrorRays` now means "reflection samples gathered per pixel, in eights" (`taps = clamp(MirrorRays, 1, 8) * 8`, read from `u_Scene.Indirect.w` in `reflection_resolve.rvshader`; Off's row set to 3 so the default keeps today's 24; the trace's dead count computation removed). Measured at the owner's camera, 1600x900, 100-frame benchmarks and 20 parked frames (`parked_stats.py r3_taps*_still`):

| taps | resolve ms | frame ms | floor parked change | floor drift 1/4/8/16 |
|---|---|---|---|---|
| 8 (Performance) | 0.58 | 10.37 | 1.20 | 1.20 / 1.84 / 2.45 / 3.18 |
| 16 (Balanced) | 1.10 | 10.68 | 1.14 | 1.14 / 1.59 / 2.02 / 2.55 |
| 24 (Off, the default) | 1.58 | 10.79 | 1.11 | 1.11 / 1.48 / 1.82 / 2.25 |
| 32 (Quality) | 2.13 | 11.73 | 1.09 | 1.09 / 1.40 / 1.70 / 2.06 |

> The resolve costs 0.065 ms a tap, linear. The drift over 16 frames falls 3.18 -> 2.55 -> 2.25 -> 2.06: each doubling buys less, as the brief predicted (the floor's disc is ~10 x 1 texels; past 16 taps they land on the same rays), but 32 still buys a little. The real presets confirm the mapping: resolve Off 1.62, Quality 2.20, Balanced 1.19, Performance 0.57 ms (their whole-frame numbers, 11.2 / 11.6 / 10.7 / 9.5, also carry the presets' shadow and AO changes). Trace unchanged at ~2.6 ms.
...
> **Tried twice on 2026-09-06 night; not shipped; the owner decides.** Attempt 1 (single-texel deviation > 4 sigma drops the history): 1.3% of the floor's texels jumped 25 levels a frame parked, drift 2.32 -> 7.30, settle 3.3 -> 8.3; the owner saw "random spotting" live -- a bright tap among 24 is many sigmas on one texel. Attempt 2 (3x3 mean deviation > 3 sigma for 3 frames, memory cut to 4): parked clean (0.002% above 25 levels), drift 2.32 -> 2.14, settle 3.29 -> 3.12, the dolly metric unchanged -- and the change test could not show any effect (curves identical with and without), because the lens switch does not reach the rays (`Switcher` sets the mesh's emissive override, which the raster reads and `ShadeTraced` does not) and the `reflection-picture` view (`patch_r6_picture.py`, C++ half applied, shader half applied, **unbuilt**) failed its first build. The owner, running the second attempt live under camera motion, reported smearing and flicker back "to some extent"; the dolly metric cannot see resets at band edges, so nothing contradicts them. Also found: with reflections off entirely, the floor sits 47 levels from its converged dark state 109 frames after the switch -- a lag in the base renderer (GI or probes; TAA's still feedback would leave ~8), noted under R9. **Recommendation:** shelve R4 -- the memory is already shortened by motion, the settle after a dolly is 3.3 levels at 60 frames, and the real-change case is dominated by the base renderer's lag.
...
#### R12 · Short memory under motion, wide blur while the history is young — ✅ accepted by the owner 2026-09-06 night (medium blur): "smearing still exists, lesser but easily observable" -- the residual is the six texels of travel the cap still averages (`kSmearTexels`), and R10 is what lets it shrink

> **Result.** Parked `moved` reads exactly 0.000 through the picture view; a 0.3 m/s hand dolly moves ~0.3 texel a frame, 1.5 m/s ~1.5. The cap: `memory <= max(kSmearTexels / moved, kMovingMemory)` with 6 texels of travel and an 8-frame floor (`reflection_accumulate.rvshader`). The blur (`reflection_blur.rvshader`, three a-trous passes at strides 1/2/4 in `FrameGraphBuilder.cpp`, `Renderer3D::BlurReflections(..., imageDistance, stride)`): radius `kYoungRadius * young^2`, bounded per axis by the lobe footprint times `1 + kYoungOverreach * young^2`; no gloss window (the footprint bound already says what a lobe allows). Measured on the 1.5 m/s dolly, floor:

| build | moving grain (HF, f70 / f100) | moving error vs converged f70 / f100 | settle f121 / f125 / f140 / f180 |
|---|---|---|---|
| before R12 | 13.5 / 12.1 | 19.7 / 19.5 | 13.0 / 10.8 / 6.6 / 3.3 |
| weak blur (radius 16, footprint-bound, gloss window) | 12.6 / 11.1 | 17.2 / 16.7 | 9.8 / 7.7 / 4.9 / 2.1 |
| **medium (radius 12, overreach 1.5) -- chosen** | 7.3 / 5.2 | 17.3 / 17.1 | 9.8 / 7.8 / 5.1 / 2.3 |
| strong (radius 24, overreach 3) | 7.1 / 4.9 | 17.8 / 18.1 | 10.6 / 8.8 / 5.1 / 2.3 |

> Parked unchanged for all (floor 1.07 a frame, drift 2.10 over 16; edges 1.40 / 1.07; converged picture identical to before). Cost: blur 0.12 ms for three passes at the owner's camera; frame 11.7 in that run (the resolve read 2.0 that run, GPU drift). The owner saw the weak build as "very noisy" and the strong one as "soft"; medium halves the grain and measures within a decimal of strong, because the footprint bound saturates the blur on the floor. **The residual moving error (17 vs 20) is the variance of one ray + 24 taps at a short history -- R10 is what lowers it further.** Both staged copies hold medium.
...
> **Built and measured 2026-09-06 late night; the premise was wrong and the real rule is found but not yet safe.** The velocity rule (the scene's velocity attachment bound to the accumulate pass at set 3 binding 6, `AtSilhouette` returning the 3x3's fastest motion, memory shortened by it at silhouettes) is in both copies: parked and dolly numbers identical to R12, harmless -- and it does nothing to the trail. The trail (`r5_sheet.png`: the car driving 1 m/s past a still camera, `BURST_SLIDE="=porsche_992_gt3_r|1.0|2.0"`, frame 100) is not at a silhouette: it is plain floor whose *reflected object* moved on, and the floor's surface passes every test. **The rule that removes it is the hit distance** (`reflection_accumulate_r5b.rvshader` in the session folder): the nearest fresh hit in the 3x3 as an image distance against the history's image distance, memory 4 when they differ by more than 2x. Visually it clears most of the ghost (`r5b_sheet.png`) but it fires on parked silhouettes too (a neighbour on another surface has another hit distance): wall edges 1.40 -> 4.05, so it is out of both copies. **Next step for R5:** take only same-surface neighbours into that minimum (the `SurfaceAt` plane/normal test the resolve already uses), skip the test where `silhouette` is true, re-measure the parked edges and the driving car. The metric against the no-history truth is unusable in absolute terms (a 40-level offset between history-on and history-off pictures); judge the trail by the sheet or build a converged-per-pose reference.
